...
- **CI** — Ручная сборка/пререлиз: `target` — строка (например `macOS Win64`); пусто = все. Один job `build` разнесён на `build-darwin`, `build-windows`, `build-win7`; каждый запускается при теге `v*` или при пустом/содержащем имя `target` (macOS, Win64, Win7). Release зависит от всех трёх (meta успешен, каждый build success/skipped, ≥1 успешен). Пререлиз: версия/тег `vX.Y.Z-N-g<sha>-prerelease` из `git describe`; в скриптах `--exclude='*-prerelease'`; теги `v.dev.*` удалены; `ci.yml` в UTF-8. В body релиза: подпись установки с версией; в блоке кода одна команда `curl ... | bash -s -- <version>`.
- **Сборка (macOS)** — В `build_darwin.sh`: переменная `APP_VERSION`; `git describe --tags --always --dirty`; fallback `unnamed-dev` при отсутствии тега.
- **Windows (TUN)** — Убрана автоматическая очистка TUN через netsh (перед стартом и при выходе): интерфейсы закрываются при завершении процесса. Парсинг конфига: общий `getConfigJSON` с удалением trailing comma для всех читателей; константы/функции для имени TUN и наличия TUN в конфиге. Добавлен отчёт по старту на Windows (`todo/windows-startup-analysis.md`) — что пробовали, что не сработало, что пробовать дальше.
...
- **CI** — Manual build/prerelease: `target` is a string (e.g. `macOS Win64`); empty = all. Single `build` job split into `build-darwin`, `build-windows`, `build-win7`; each runs on tag `v*` or when `target` is empty/contains name (macOS, Win64, Win7). Release depends on all three (meta success, each build success/skipped, ≥1 success). Prerelease: version/tag `vX.Y.Z-N-g<sha>-prerelease` from `git describe`; scripts use `--exclude='*-prerelease'`; legacy `v.dev.*` removed; `ci.yml` UTF-8. Release body: install caption with version; code block has single `curl ... | bash -s -- <version>`.
- **Build (macOS)** — `build_darwin.sh`: `APP_VERSION` env var; `git describe --tags --always --dirty`; fallback `unnamed-dev` when no tag.
- **Windows (TUN)** — Removed automatic TUN cleanup via netsh (before start and on exit); interfaces are closed when the process exits. Config parsing: shared `getConfigJSON` with trailing-comma removal for all readers; helpers for TUN interface name and ConfigHasTun. Added Windows startup analysis doc (`todo/windows-startup-analysis.md`) — what was tried, what failed, next steps.
