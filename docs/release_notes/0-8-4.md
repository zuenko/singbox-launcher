...
- **Визард: платформа win7:** В шаблоне визарда (params и selectable_rules) при сборке Win7 из CI (GOARCH=386) применяются и секции с `"platforms": ["windows"]`, и с `"platforms": ["win7"]` — по аналогии с несколькими метками платформы на macOS/Linux.
