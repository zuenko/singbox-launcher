...
### 2.2. Release notes — два пути

CI сначала ищет `docs/release_notes/<SLUG>.md` (путь из 2.1). Если файла нет — для пререлизов CI **автоматически фоллбэкается на `docs/release_notes/upcoming.md`** (для stable-тегов фоллбэка нет, файл обязателен).

**Почему есть fallback.** Slug пререлиза содержит short-SHA текущего HEAD. Любой commit (в том числе с release-notes файлом) меняет HEAD, и slug становится новым — файл, который вы только что закоммитили под старый slug, оказывается «не той версии» и CI его не находит. Структурный chicken-and-egg, решается fallback'ом.

**Путь А — ничего не делать (типичный случай).** На `develop` уже есть актуальный `upcoming.md` с пунктами по всем фичам/багам после последнего stable. Достаточно убедиться что он отражает то, что хочется в пререлиз, и сразу переходить к 2.3 — CI возьмёт его.

**Путь Б — отдельный per-prerelease файл (опционально).** Если хочется чтобы в release-теле было что-то отличное от текущего черновика (например, узкий QA-build под конкретную фичу) — создайте `docs/release_notes/<SLUG>.md`:

1. Содержимое: 1–3 пункта что нового в этом пререлизе поверх последнего stable. Формат:
   ```markdown
   ## Highlights (EN)
   ...
   ```
2. Коммит на `develop`: `docs(release): notes for <SLUG>`. Запушить.
3. **Важно:** после commit slug сместится (HEAD изменился). Точное попадание под новый slug — это redo рутина с rename + amend; на практике проще положиться на fallback из Пути А либо принять что точный файл может не сматчиться → CI всё равно возьмёт `upcoming.md`.
